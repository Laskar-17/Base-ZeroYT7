...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    zeroyt7.on('open', () => {
        success('2', 'Terhubung Jangan Lupa Support Laskar17')
    })
...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya Laskar17 BOT saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk meulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, wel, "©Created : Laskar17", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
        memeg = mdata.participants.length
        out = `Bye Tod \nGoodbye @${anu_user} Semoga Tenang Di Alam Sana Tod`
        buff = await getBuffer(
        ...
        sendButImage(mdata.id, out, "©Created : Laskar17", buff, but)
      }
